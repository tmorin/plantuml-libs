...
This guide provides concrete, step-by-step instructions to upgrade the Font Awesome package in the plantuml-libs repository. It follows the [Diátaxis How-to Guide](https://diataxis.fr/how-to-guides/) style and is designed for AI agents to execute.
...
```
create_branch(owner="tmorin", repo="plantuml-libs", branch="feat/upgrade-fontawesome-icons", from_branch="master")
```
...
```
push_files(owner="tmorin", repo="plantuml-libs", branch="feat/upgrade-fontawesome-icons", 
  files=[...], message="feat(fontawesome): update icons\n\nUpdated Font Awesome package with latest available icons from the official repository.\n\nCo-authored-by: Copilot <223556219+Copilot@users.noreply.github.com>")
```

**Fallback (CLI)**:
```bash
git add .
git commit -m "feat(fontawesome): update icons
...
create_dispatch_event(owner="tmorin", repo="plantuml-libs", event_type="package-builder", 
  client_payload={"pkgName": "fontawesome", "pkgVersion": "latest", "branch": "feat/upgrade-fontawesome-icons"})
```
...
trigger_workflow(owner="tmorin", repo="plantuml-libs", workflow_id="package-builder.yaml",
  inputs={"pkgName": "fontawesome", "pkgVersion": "latest"}, ref="feat/upgrade-fontawesome-icons")
```
...
create_pull_request(owner="tmorin", repo="plantuml-libs", 
  ...
  head="feat/upgrade-fontawesome-icons", 
  base="master",
