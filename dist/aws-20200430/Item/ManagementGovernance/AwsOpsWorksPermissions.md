# Aws Ops Works Permissions

```text
aws-20200430/Item/ManagementGovernance/AwsOpsWorksPermissions
```

```text
include('aws-20200430/Item/ManagementGovernance/AwsOpsWorksPermissions')
```

|icon|card|element|group|
|---|---|---|---|
|![](AwsOpsWorksPermissions.png)|![](AwsOpsWorksPermissions.card.png)|![](AwsOpsWorksPermissions.element.png)|![](AwsOpsWorksPermissions.group.png)|



## card
### Load remotely
```plantuml
@startuml
' configures the library
!global $LIB_BASE_LOCATION="https://raw.githubusercontent.com/tmorin/plantuml-libs/master/dist"
' loads the library
!include $LIB_BASE_LOCATION/bootstrap.puml
' loads the aws-20200430 bootstrap
include('aws-20200430/bootstrap')
' loads the AwsOpsWorksPermissions element
include('aws-20200430/Item/ManagementGovernance/AwsOpsWorksPermissions')
AwsOpsWorksPermissionsCard('aws_ops_works_permissions', 'Aws Ops Works Permissions', 'Molestiae praesentium aut natus dolorem. Aut consequatur fugiat a consequatur ut.')
@enduml
```
### Load locally
```plantuml
@startuml
' configures the library
!global $INCLUSION_MODE="local"
!global $LIB_BASE_LOCATION="../../.."
' loads the library
!include $LIB_BASE_LOCATION/bootstrap.puml
' loads the aws-20200430 bootstrap
include('aws-20200430/bootstrap')
' loads the AwsOpsWorksPermissions element
include('aws-20200430/Item/ManagementGovernance/AwsOpsWorksPermissions')
AwsOpsWorksPermissionsCard('aws_ops_works_permissions', 'Aws Ops Works Permissions', 'Molestiae praesentium aut natus dolorem. Aut consequatur fugiat a consequatur ut.')
@enduml
```


## element
### Load remotely
```plantuml
@startuml
' configures the library
!global $LIB_BASE_LOCATION="https://raw.githubusercontent.com/tmorin/plantuml-libs/master/dist"
' loads the library
!include $LIB_BASE_LOCATION/bootstrap.puml
' loads the aws-20200430 bootstrap
include('aws-20200430/bootstrap')
' loads the AwsOpsWorksPermissions element
include('aws-20200430/Item/ManagementGovernance/AwsOpsWorksPermissions')
AwsOpsWorksPermissions('aws_ops_works_permissions', 'Aws Ops Works Permissions', 'an optional tech field')
@enduml
```
### Load locally
```plantuml
@startuml
' configures the library
!global $INCLUSION_MODE="local"
!global $LIB_BASE_LOCATION="../../.."
' loads the library
!include $LIB_BASE_LOCATION/bootstrap.puml
' loads the aws-20200430 bootstrap
include('aws-20200430/bootstrap')
' loads the AwsOpsWorksPermissions element
include('aws-20200430/Item/ManagementGovernance/AwsOpsWorksPermissions')
AwsOpsWorksPermissions('aws_ops_works_permissions', 'Aws Ops Works Permissions', 'an optional tech field')
@enduml
```


## group
### Load remotely
```plantuml
@startuml
' configures the library
!global $LIB_BASE_LOCATION="https://raw.githubusercontent.com/tmorin/plantuml-libs/master/dist"
' loads the library
!include $LIB_BASE_LOCATION/bootstrap.puml
' loads the aws-20200430 bootstrap
include('aws-20200430/bootstrap')
' loads the AwsOpsWorksPermissions element
include('aws-20200430/Item/ManagementGovernance/AwsOpsWorksPermissions')
AwsOpsWorksPermissionsGroup('aws_ops_works_permissions', 'Aws Ops Works Permissions', 'an optional tech field'){
note as note
the content of the group
end note
}
@enduml
```
### Load locally
```plantuml
@startuml
' configures the library
!global $INCLUSION_MODE="local"
!global $LIB_BASE_LOCATION="../../.."
' loads the library
!include $LIB_BASE_LOCATION/bootstrap.puml
' loads the aws-20200430 bootstrap
include('aws-20200430/bootstrap')
' loads the AwsOpsWorksPermissions element
include('aws-20200430/Item/ManagementGovernance/AwsOpsWorksPermissions')
AwsOpsWorksPermissionsGroup('aws_ops_works_permissions', 'Aws Ops Works Permissions', 'an optional tech field'){
note as note
the content of the group
end note
}
@enduml
```

