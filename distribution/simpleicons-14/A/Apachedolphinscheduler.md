# Apachedolphinscheduler


```text
simpleicons-14/A/Apachedolphinscheduler
```

```text
include('simpleicons-14/A/Apachedolphinscheduler')
```



| Illustration | Apachedolphinscheduler |
| :---: | :---: |
| ![illustration for Illustration](../../simpleicons-14/A/Apachedolphinscheduler.png) | ![illustration for Apachedolphinscheduler](../../simpleicons-14/A/Apachedolphinscheduler.Local.png) |



## Sprites
The item provides the following sriptes:

- `<$ApachedolphinschedulerXs>`
- `<$ApachedolphinschedulerSm>`
- `<$ApachedolphinschedulerMd>`
- `<$ApachedolphinschedulerLg>`





## Apachedolphinscheduler

### Load remotely
```plantuml
@startuml
' configures the library
!global $LIB_BASE_LOCATION="https://raw.githubusercontent.com/tmorin/plantuml-libs/master/distribution"

' loads the library's bootstrap
!include $LIB_BASE_LOCATION/bootstrap.puml

' loads the package bootstrap
include('simpleicons-14/bootstrap')

' loads the Item which embeds the element Apachedolphinscheduler
include('simpleicons-14/A/Apachedolphinscheduler')

' renders the element
Apachedolphinscheduler('Apachedolphinscheduler', 'Apachedolphinscheduler', 'an optional tech label', 'an optional description')
@enduml
```

### Load locally
```plantuml
@startuml
' configures the library
!global $INCLUSION_MODE="local"
!global $LIB_BASE_LOCATION="../.."

' loads the library's bootstrap
!include $LIB_BASE_LOCATION/bootstrap.puml

' loads the package bootstrap
include('simpleicons-14/bootstrap')

' loads the Item which embeds the element Apachedolphinscheduler
include('simpleicons-14/A/Apachedolphinscheduler')

' renders the element
Apachedolphinscheduler('Apachedolphinscheduler', 'Apachedolphinscheduler', 'an optional tech label', 'an optional description')
@enduml
```

