...
export function getAbsoluteImagePath(context: PackageContext, cwd: string, relativeImagePathToGlob: string) {
    return P.isAbsolute(context.dstYamlPath) ? P.join(cwd, relativeImagePathToGlob) : P.join(process.cwd(), cwd, relativeImagePathToGlob);
}

export function getAbsoluteDstYamlDirPath(workDirPath: string) {
    const dstYamlPath = getTmpDirPath(workDirPath);
    return P.dirname(P.isAbsolute(dstYamlPath) ? dstYamlPath : P.join(process.cwd(), dstYamlPath));
}
...
export async function render(source: string, destination: string, data: object) {
    if (!TEMPLATES[source]) {
