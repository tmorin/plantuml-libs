...
const iconsVersion='5.15.3'
const iconsUrl = `https://github.com/FortAwesome/Font-Awesome/releases/download/${iconsVersion}/fontawesome-free-${iconsVersion}-web.zip`
...
async function createInventory() {
    ...
    const cwd = P.join(iconsZipDst, `fontawesome-free-${iconsVersion}-web`);

    glob.sync(
        'svgs/**/font-awesome-logo-full.svg',
        {cwd, nodir: true}
    ).forEach(path => F.unlinkSync(P.join(cwd, path)))
