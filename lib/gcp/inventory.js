...
// https://docs.google.com/presentation/d/1aGOTpNdCoO4GXZ2es38ZFO5qPGEAjTtDSVeHaDpwsas
const iconsUrl = "https://cloud.google.com/icons/files/google-cloud-icons.zip"
...
async function createInventory() {
    ...
    const workMainLibDir = P.join(iconsZipDst, 'Google Cloud Icons (updated)')
    if (F.existsSync(P.join(workMainLibDir, 'Products & Services'))) {
        F.renameSync(P.join(workMainLibDir, 'Products & Services'), P.join(workMainLibDir, 'Item'));
    }
    ...
    // group.forEach(entry => console.log('%s', entry.element.urn))
    // process.exit()
