...
async function renderPuml(source) {
    ...
    await exec(`${getConfig().javaCmd} -Djava.awt.headless=true -jar ${plantumlJarPath} ${source}`, {
...
async function encodeSprite(source, destination) {
    ...
    await exec(`${getConfig().javaCmd} -Djava.awt.headless=true -jar ${plantumlJarPath} -encodesprite 16z ${source} > ${destination}`, {
        stdio: process.stdout
    }).catch(error => log.warn(error.message));
}
