...
import { render } from "../../../generator/workdir/paths"
...
async function writeAllElementTemplates(
  context: PackageContext,
  element_items: Array<Item>
) {
...
export class EventstormingFactory implements PackageFactory {
  ...
  async create(context: PackageContext): Promise<Package> {
    const element_items_as_csv: Array<ElementItemCsvRow> = parse(
      F.readFileSync(P.join(__dirname, "elements.csv"), { encoding: "utf-8" }),
      {
        columns: true,
      }
    )
    const element_items: Array<Item> = element_items_as_csv.map(
      ({
        family,
        name,
        description,
