...
const ICONS_URL = "https://d1.awsstatic.com/webteam/architecture-icons/q3-2021/Asset-Package_07302021.533e2e5c12d0759fd00ce35fa70d8418a26f4a90.zip"
...
export class Aws20210730Factory implements PackageFactory {
    getUrn(): string {
        return "aws-20210730";
    }
    ...
    async create(context: PackageContext): Promise<Package> {
        ...
        await Fe.copy(
            P.join(iconsZipDst, 'Asset-Package_07302021', 'Architecture-Service-Icons_07302021'),
            P.join(iconsDst, 'architecture'),
            {overwrite: false, recursive: true}
        )
        await Fe.mkdir(P.join(iconsDst, 'category'), {recursive: true})
        await Fe.copy(
            P.join(iconsZipDst, 'Asset-Package_07302021', 'Category-Icons_07302021'),
            P.join(iconsDst, 'category'),
            {overwrite: false, recursive: true}
        )
        await Fe.mkdir(P.join(iconsDst, 'resource'), {recursive: true})
        await Fe.copy(
            P.join(iconsZipDst, 'Asset-Package_07302021', 'Resource-Icons_07302021'),
